...
# Improved CP Violation Diagnostics (direct MT echo, Dec 23 2025)
def calculate_cp_violation(mean_deg, lock_err_deg, kappa_grad_sq):
    """
    Direct proxy: average cos(Δθ) → cos(255°) = -0.2588 when locked.
    Small boost from coherence (1 - kappa normalized) for dynamics.
    Kappa unchanged.
    """
    mean_rad = mean_deg * np.pi / 180.0
    cp_asym_direct = np.cos(mean_rad)  # Core echo
    # Optional mild coherence boost (kappa low → fuller echo)
    coherence_factor = 1.0 - np.tanh(kappa_grad_sq / 1e-3)  # Tunable, near 1 when gradients flat
    cp_asym = cp_asym_direct * coherence_factor
    return cp_asym, kappa_grad_sq
...
def run_full_simulation(
    ...
    prev_mean_c = 0.0  # For dΔθ/dt in baryogenesis proxy
    ...
    for step in range(iterations):
        ...
        if monitor_every and (step % monitor_every == 0):
            ...
            # NEW: Gradient kappa for decoherence
            kappa = float(np.mean(grad_sq_periodic(dtheta_now, dx)))
            # Baryogenesis proxy (MT v2 style)
            if step > 0:
                dtheta_dt = (mean_c - prev_mean_c) / (monitor_every * dt)
            else:
                dtheta_dt = 0.0
            chiral_bias = np.sin(mean_c - target_delta_0_rad)  # Direction from 255° offset
            baryo_rate = dtheta_dt * chiral_bias * kappa  # Instant rate ∝ change × bias × decoherence
            # Cumulative (freezes as kappa→0)
            if step == 0:
                cum_baryo = 0.0
            else:
            ...
            cp_asym, _ = calculate_cp_violation(mean_deg, err_deg, kappa)
            ...
            if store_series:
                ...
                series["cp_asymmetry"] = series.get("cp_asymmetry", []) + [cp_asym]
                series["kappa_term"] = series.get("kappa_term", []) + [kappa]
                series["baryo_rate"] = series.get("baryo_rate", []) + [baryo_rate]
                series["cum_baryo"] = series.get("cum_baryo", []) + [cum_baryo]
...
if __name__ == "__main__": # Corrected from _name_
 ...
 # Initialize fields near target lock
    ...
    if "cum_baryo" in series and series["cum_baryo"]:
        final_eta_raw = series["cum_baryo"][-1]
        # Toy scaling to observed ballpark (tune factor for ~6e-10)
        eta_toy = abs(final_eta_raw) * 1e9  # Adjust multiplier as needed
        print(f"Toy Baryon Asymmetry η ≈ {eta_toy:.2e} (target ~6e-10)")
    ...
    if series is not None and len(series["t"]) > 2: # Ensure enough data for plotting
        ...
        if "cp_asymmetry" in series and len(series["cp_asymmetry"]) > 2:
            plt.figure(figsize=(10, 6))
            plt.plot(t, series["cp_asymmetry"], label="CP Asymmetry Proxy", color="purple", linewidth=2)
            plt.axhline(-0.258819, color="gray", linestyle="--", label="cos(255°) = -0.258819")
            plt.plot(t, series["kappa_term"], label="Kappa Term ⟨|∇Δθ|²⟩", color="orange", alpha=0.7)
            plt.title("CP Violation Echo & Coherence Growth")
            plt.xlabel("Time (s)")
            plt.ylabel("Value")
            plt.legend()
        ...
        # Baryogenesis Diagnostics Plot
        if "baryo_rate" in series and "cum_baryo" in series and len(series["baryo_rate"]) > 2:
            plt.figure(figsize=(10, 6))
            plt.plot(t, series["baryo_rate"], label="Instant Baryogenesis Rate", color="teal", alpha=0.7)
            plt.plot(t, series["cum_baryo"], label="Cumulative η_toy (raw)", color="darkblue", linewidth=2.5)
            plt.axhline(0, color="gray", linestyle="--", alpha=0.5)
            plt.title("Baryogenesis from Phase Relaxation (MT v2 Chain Proxy)")
            plt.xlabel("Time (s)")
            plt.ylabel("Rate (toy) / Cumulative Asymmetry")
            plt.legend()
            plt.grid(True, alpha=0.3)
            plt.show()
            ...
            # Scaled final η
            final_eta_raw = series["cum_baryo"][-1]
            eta_toy_scaled = final_eta_raw * 1e10  # Tune this multiplier for ~6e-10 ballpark
            print(f"Toy Scaled Baryon Asymmetry η ≈ {eta_toy_scaled:.2e} (observed ~6×10⁻¹⁰)")

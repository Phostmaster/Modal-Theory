...
class GEMParams:
    ...
    damage_threshold: float = 0.35
    damage_rate: float = 0.025
    nonlinear_alpha: float = 1.0

    residual_softening: float = 0.7
    residual_viscosity_gain: float = 0.6

    memory_gain: float = 0.035
    memory_decay_scale: float = 0.008

    edge_width_factor: float = 0.15
    edge_softening_strength: float = 0.5

    brittle_gain: float = 0.06          # extra damage multiplier for brittle snap
    brittle_depth_floor: float = 0.65   # brittle spike activates when D_eff / D falls below this
...
class GEMPolymerSim:
    ...
    def brittle_damage_multiplier(self, total_x: float, kappa_eff: float, D_eff: float) -> float:
        ...
        stiffness_factor = max(0.0, stiffness_ratio - 0.85)
    ...
    def step(self, step_idx: int) -> Dict[str, float]:
        ...
        # Recompute visible deformation
        total_x = self.x + self.memory

        # Recoverable memory channel
        memory_inflow = self.p.memory_gain * self.p.viscosity * abs(self.x) * 0.05
        memory_decay = self.memory * 0.012
        self.memory += memory_inflow - memory_decay
        self.memory = max(0.0, self.memory)
        # Recompute total deformation after memory update
        total_x = self.x + self.memory

        # Irreversible damage only near / beyond basin edge
        excess = max(0.0, abs(total_x) - self.p.damage_threshold)
        if excess > 0.0:
            edge_factor = 1.0 + excess / max(D_eff, 1e-9)
...
def classify_run(results: List[Dict[str, float]], params: GEMParams) -> str:
    ...
    # Brittle collapse: high stiffness + low depth + early failure
    if failed == 1:
        frac = len(results) / max(params.cycles * params.cycle_period, 1)
        if frac < 0.25 and last.get("effective_stiffness", 0) > 0.7:
            return "brittle collapse"
        return "collapse"

    # Trapped metastable: high memory with low damage
    if memory_final > 0.12 and damage_final < 0.08:
        return "trapped metastable"

    # Creeping distortion: significant damage accumulation (slow failure)
    if damage_final > 0.15 * params.depth:
        return "creeping distortion"

    # Clean recovery: high persistence, low damage and memory
    if persistence_final > 0.5 and damage_final < 0.05 * params.depth and memory_final < 0.05 * params.depth:
        return "clean recovery"

    # Stable with drift: some memory but not failing
    if abs(total_def_final) > 0.1 or memory_final > 0.05 * params.depth:
...
def run_quick_comparison() -> None:
    presets = [
        ("brittle_like", GEMParams(depth=0.30, stiffness=0.98, viscosity=0.25, cycles=600, run_name="brittle_like", brittle_gain=0.10)),
        ("creep_like", GEMParams(depth=0.75, stiffness=0.25, viscosity=1.30, cycles=2000, run_name="creep_like")),
        ("metastable_like", GEMParams(depth=1.00, stiffness=0.60, viscosity=1.40, cycles=1500, run_name="metastable_like")),
        ("balanced", GEMParams(depth=0.85, stiffness=0.70, viscosity=0.50, cycles=1000, run_name="balanced")),
    ]
...
def axis_scan(
    ...
    for G in viscosity_values:
        ...
        plt.colorbar(label="0 collapse ... 5 clean recovery")
        plt.tight_layout()
...
if __name__ == "__main__":
    ...
    base = GEMParams(
        ...
        cycle_period=50,
        load_amplitude=0.8,
        damage_threshold=0.35,
        damage_rate=0.025,
        memory_gain=0.035,
        memory_decay_scale=0.008,
        brittle_gain=0.12,
